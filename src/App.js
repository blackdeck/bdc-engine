...
class App extends Component {
  ...
  playGame() {
    ...
    this.timerID = setInterval(
      () => this.frame(),
      Math.floor(
        this.state.game_speed /
          this.state.frame_rate /
          this.state.game_speed_multiplier
      )
    );
  ...
  onClickWrapper(item) {
    if (item.cost) {
      if (this.isEnough(this.state, item.cost)) {
        if (item.onClick)
          this.setState(item.onClick(this.chargeCost(this.state, item.cost)));
      } else {
        return false;
      }
    } else {
  ...
  createPopup() {
    ...
    this.popupHandler.createPopup(
      `POPUP №${this.i}`,
      <div>{'This is... You guessed it. A POPUP!!!'}</div>
    );
  }
      ...
      <div className="App">
        ...
        <div className="flex-element flex-container-row">
          <div className="flex-element">
            <h4>
              Tick: {this.state.tick} Frame: {this.state.frame}{' '}
            </h4>
                      ...
                      <button
                        className={classNames(
                          this.state.mode === key
                            ...
                            : 'btn-warning',
                          item.cost
                            ? this.isEnough(this.state, item.cost)
                              ? ''
                              : 'disabled'
                            : ''
                        )}
          ...
          <div className="flex-element">{this.state.matrix_show}</div>
        </div>
        ...
        <div className="flex-container-row">
          <div
            className="flex-element flex-container-column"
            style={{ height: '100%' }}
          >
            <div className="flex-element flex-container-row">
              {_.map(
                ...
                (item, key) =>
                  item.locked && item.locked(this.state) ? (
                    ...
                    >
                        ...
                        >
                          {
                            <button
                              className={classNames(
                                this.state[key].current_state === 'stopped'
                                  ? 'btn-danger'
                                  : this.state[key].next_command === 'start'
                                    ? 'btn-success'
                                    : 'btn-warning',
                                item.cost
                                  ? this.isEnough(this.state, item.cost)
                      ...
                      <div className="flex-element">
                        Cooldown: {state[key].cooldown_timer} /{' '}
                        {state[key].cooldown}
                      </div>
                      <div className="flex-element">
        ...
        <div className="flex-element flex-container-row">
          <div className="flex-element">
            ...
            <h4>
              Armor: {this.state.player.armor_current} /{' '}
          ...
          <div className="flex-element">
            <h2>Target</h2>
            <h4>
              {this.state.target.name} lvl {this.state.target.level}{' '}
            </h4>
            <h4>Weapon: {this.state.target.dmg}</h4>
            <h4>
              Armor: {this.state.target.armor_current} /{' '}
