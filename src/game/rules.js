...
export const rules = {
  ...
  cards_rule: { onTick: (state) => { if (state.ideas > 4 && _.random(1, 2) === 1) { state.ideas -= 4; state.cards++; } return state; } },
  decks_rule: { onTick: (state) => { if (state.cards > 4 && _.random(1, 16) === 1) { state.cards -= 4; state.decks++; } return state; } },
  games_rule: { onTick: (state) => { if (state.decks > 4 && _.random(1, 128) === 1) { state.decks -= 4; state.games++; } return state; } },
};
